...
$code = apply_filters( 'wpml_translate_single_string', $code, self::CONTEXT_WPML, $this->field_name_translation( 'code' ) );
...
if ( ! empty( $code ) ) {
    ...
    $imgs = [];
    if ( $phone ) {
        $phone = apply_filters( 'wpml_translate_single_string', $phone, self::CONTEXT_WPML, $this->field_name_translation( 'phone' ) );
        $imgs[] = wp_get_attachment_image( $phone, 'full', false, [ 'class' => 'banner56--mobile' ]);
    }
    if ( $tablet ) {
        $tablet = apply_filters( 'wpml_translate_single_string', $tablet, self::CONTEXT_WPML, $this->field_name_translation( 'tablet' ) );
