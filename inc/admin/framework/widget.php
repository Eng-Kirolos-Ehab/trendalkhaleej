...
{
    const CONTEXT_WPML = 'Widgets';
    // placeholder function
    function fields() {
        return array();
    }

    function field_name_translation( $field_id, $num_order = '' ) {
        if( !$num_order ) $num_order = $this->number;
        return $this->name.'('. $num_order .')-'. strval( $field_id );
    }
    ...
    function update ( $new_instance, $old_instance ) {
        ...
        foreach ( (array) $this->fields() as $field ) {
            ...
            if ( $type != 'group' ) {
                $instance[ $field['id'] ] = $this->update_field( $new_instance, $field['id'], $type, isset( $field['translate'] ) );
            } else {
     ...
     * @since 1.0
     */
    function update_field( $new_instance, $key, $type, $translation = false ) {
        $return = null;
        if ( $type == 'text' ) {
        ...
        if( $translation ) {
            do_action( 'wpml_register_single_string', self::CONTEXT_WPML, $this->field_name_translation( $key ), $return );
        }
        return $return;
    }
