...
"use strict";
    ...
    WITHEMES_ADMIN.post_format = function() {
        let default_format_ui = $( '#post-formats-select' )
        if ( ! default_format_ui.length ) {
            return
        }
        $( 'select[name="post_format"]' ).on( 'change', function() {
            let v = $( this ).val()
            if ( ! v ) {
                v = '0'
            }
            default_format_ui.find('input[type="radio"]').filter('[value=' + v + ']').prop('checked', true);
        })
    }
    ...
    WITHEMES_ADMIN.custom_sidebar = function() {
        ...
        $( '.fox-wrap' ).on( 'click', '.fox-add-sidebar, .fox-remove-sidebar', function( e ) {
